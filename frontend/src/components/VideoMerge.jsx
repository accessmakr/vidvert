...
const ALLOWED=/\.(mp4|mkv|webm|avi|mov|wmv|flv|3gp|m4v)$/i;
...
function uploadMultipleVideos(files,onProgress){
  return new Promise((resolve,reject)=>{
    const form=new FormData();
    files.forEach(f=>form.append('files',f));
    const xhr=new XMLHttpRequest();
    xhr.upload.onprogress=e=>{if(e.lengthComputable)onProgress(Math.round((e.loaded/e.total)*100));};
    xhr.onload=()=>{try{const d=JSON.parse(xhr.responseText);if(xhr.status>=200&&xhr.status<300)resolve(d);else reject(new Error(d.error||'Upload failed'));}catch{reject(new Error('Upload failed ('+xhr.status+')'));}};
    xhr.onerror=()=>reject(new Error('Network error'));
    xhr.ontimeout=()=>reject(new Error('Upload timed out'));
    xhr.timeout=10*60*1000;
    xhr.open('POST',CONVERTER_URL+'/merge-videos');
    xhr.send(form);
  });
}
...
export default function VideoMerge() {
  ...
  const addFiles=e=>{const newFiles=Array.from(e.target.files||[]).filter(f=>ALLOWED.test(f.name));setFiles(prev=>[...prev,...newFiles]);e.target.value='';};
  const removeFile=i=>setFiles(prev=>prev.filter((_,idx)=>idx!==i));
  const start=async()=>{if(files.length<2){setErr('Add at least 2 video files to merge.');return;}reset();setPhase('uploading');setPct(0);
    try{const d=await uploadMultipleVideos(files,p=>setPct(p));setJobId(d.jobId);setPhase('converting');}
    catch(e){setPhase('error');setErr(e.message);}};
  const progress=phase==='uploading'?pct:(job?.progress||0);

  return(
    <section className="w-full max-w-xl flex flex-col gap-4" aria-label="Video Merge">
      <div><h2 className="text-white font-bold text-base">Video Merge</h2><p className="text-zinc-500 text-xs mt-0.5">Join multiple video clips into one. Videos are concatenated in the order shown.</p></div>
      <div onDrop={e=>{e.preventDefault();setDrag(false);const f=Array.from(e.dataTransfer.files).filter(f=>ALLOWED.test(f.name));setFiles(prev=>[...prev,...f]);}} onDragOver={e=>{e.preventDefault();setDrag(true);}} onDragLeave={()=>setDrag(false)} onClick={()=>!working&&ref.current?.click()} role="button" tabIndex={0}
        className={'border-2 border-dashed rounded-xl p-4 text-center cursor-pointer transition-all '+(drag?'border-violet-400 bg-violet-950/20':'border-zinc-700 hover:border-zinc-500 bg-zinc-900/50')+(working?' pointer-events-none opacity-50':'')}>
        <input ref={ref} type="file" multiple accept="video/*" onChange={addFiles} className="hidden"/>
        <p className="text-zinc-400 text-sm font-medium">{drag?'Drop videos here':'Tap to add video files'}</p>
        <p className="text-zinc-600 text-xs mt-1">You can add multiple files</p>
      </div>
      {files.length>0&&<div className="flex flex-col gap-1.5">
        {files.map((f,i)=>(
          <div key={i} className="flex items-center gap-2 bg-zinc-900 border border-zinc-800 rounded-xl px-3 py-2.5">
            ...
            {!working&&<button onClick={()=>removeFile(i)} className="text-zinc-600 hover:text-zinc-400 flex-shrink-0 text-xs">✕</button>}
          </div>
        ))}
      ...
      {!done&&<button onClick={start} disabled={working||files.length<2} className="w-full bg-violet-600 hover:bg-violet-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-semibold py-3 rounded-xl transition-colors">{working?(phase==='uploading'?'Uploading… '+pct+'%':(job?.statusText||'Merging…')):'Merge '+files.length+' Videos'}</button>}
      {working&&<div className="flex flex-col gap-1.5"><div className="flex justify-between"><span className="text-zinc-300 text-xs">{phase==='uploading'?'Uploading… '+pct+'%':(job?.statusText||'Merging…')}</span><span className="text-zinc-500 text-xs">{progress>0?progress+'%':''}</span></div><div className="w-full bg-zinc-800 rounded-full h-2 overflow-hidden"><div className={'h-2 '+(phase==='uploading'?'bg-blue-500':'bg-violet-500')+' rounded-full transition-all duration-500'} style={{width:progress+'%'}}/></div></div>}
      {done&&<div className="flex flex-col gap-2"><button onClick={()=>save(getConverterDownloadUrl(job.jobId),'merged.mp4')} className="w-full bg-green-600 hover:bg-green-500 text-white font-semibold py-3 rounded-xl transition-colors">✓ Download Merged Video{job?.fileSizeBytes?' ('+formatBytes(job.fileSizeBytes)+')':''}</button><button onClick={()=>{reset();setFiles([]);}} className="text-zinc-500 hover:text-zinc-300 text-xs text-center py-1">Merge another batch</button></div>}
