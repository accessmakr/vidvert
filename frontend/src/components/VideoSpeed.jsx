...
const SPEEDS=[{value:'0.25',label:'0.25×',desc:'Slow motion'},{value:'0.5',label:'0.5×',desc:'Half speed'},{value:'1.5',label:'1.5×',desc:'Faster'},{value:'2',label:'2×',desc:'Double speed'},{value:'4',label:'4×',desc:'4× fast'}];
...
export default function VideoSpeed() {
  const [file,setFile]=useState(null);const [drag,setDrag]=useState(false);const [speed,setSpeed]=useState('2');
  ...
  const start=async()=>{reset();setPhase('uploading');setPct(0);
    try{const form=new FormData();form.append('file',file);form.append('speed',speed);form.append('filename',file.name.replace(/\.[^.]+$/,''));
      const d=await uploadVideoForProcessing('speed-video',form,p=>setPct(p));setJobId(d.jobId);setPhase('converting');
    }catch(e){setPhase('error');setErr(e.message);}};
  const progress=phase==='uploading'?pct:(job?.progress||0);

  return(
    <section className="w-full max-w-xl flex flex-col gap-4" aria-label="Video Speed">
      <div><h2 className="text-white font-bold text-base">Video Speed Control</h2><p className="text-zinc-500 text-xs mt-0.5">Speed up or slow down any video. Audio pitch adjusts automatically.</p></div>
      <div onDrop={e=>{e.preventDefault();setDrag(false);accept(e.dataTransfer.files?.[0]);}} onDragOver={e=>{e.preventDefault();setDrag(true);}} onDragLeave={()=>setDrag(false)} onClick={()=>!working&&ref.current?.click()} role="button" tabIndex={0} className={'border-2 border-dashed rounded-xl p-5 text-center cursor-pointer transition-all '+(drag?'border-lime-400 bg-lime-950/20':file?'border-zinc-600 bg-zinc-900':'border-zinc-700 hover:border-zinc-500 bg-zinc-900/50')+(working?' pointer-events-none opacity-50':'')}>
      ...
      <div><p className="text-zinc-400 text-xs mb-2 font-medium uppercase tracking-wide">Speed</p>
        <div className="grid grid-cols-5 gap-2">{SPEEDS.map(s=>(
          <button key={s.value} onClick={()=>!working&&setSpeed(s.value)} disabled={working} aria-pressed={speed===s.value}
            className={'flex flex-col items-center py-2.5 rounded-xl border text-center transition-all disabled:opacity-50 '+(speed===s.value?'border-lime-500 bg-lime-950 text-white':'border-zinc-700 bg-zinc-900 text-zinc-400 hover:border-zinc-500')}>
            <span className="font-bold text-sm">{s.label}</span><span className="text-xs opacity-60 mt-0.5">{s.desc}</span>
          </button>))}
        </div>
      </div>
      {!done&&<button onClick={start} disabled={working||!file} className="w-full bg-lime-600 hover:bg-lime-500 disabled:opacity-40 disabled:cursor-not-allowed text-white font-semibold py-3 rounded-xl transition-colors">{working?(phase==='uploading'?'Uploading… '+pct+'%':(job?.statusText||'Processing…')):('Apply '+speed+'× Speed')}</button>}
      {working&&<div className="flex flex-col gap-1.5"><div className="flex justify-between"><span className="text-zinc-300 text-xs">{phase==='uploading'?'Uploading… '+pct+'%':(job?.statusText||'Processing…')}</span><span className="text-zinc-500 text-xs">{progress>0?progress+'%':''}</span></div><div className="w-full bg-zinc-800 rounded-full h-2 overflow-hidden"><div className={'h-2 '+(phase==='uploading'?'bg-blue-500':'bg-lime-500')+' rounded-full transition-all duration-500'} style={{width:progress+'%'}}/></div></div>}
      {done&&<div className="flex flex-col gap-2"><button onClick={()=>save(getConverterDownloadUrl(job.jobId),'video_x'+speed+'.mp4')} className="w-full bg-green-600 hover:bg-green-500 text-white font-semibold py-3 rounded-xl transition-colors">✓ Download {speed}× Video{job?.fileSizeBytes?' ('+formatBytes(job.fileSizeBytes)+')':''}</button><button onClick={()=>{reset();setFile(null);}} className="text-zinc-500 hover:text-zinc-300 text-xs text-center py-1">Adjust another file</button></div>}
